...
public class LaptopClient {
    ...
    public void uploadImage(String laptopID, String imagePath) throws InterruptedException {
        ...
        if (!finishLatch.await(1, TimeUnit.MINUTES)) {
            logger.warning("request cannot finishw within 1 minute");
        }
                ...
                .rateLaptop(new StreamObserver<RateLaptopResponse>() {
                    ...
                    public void onNext(RateLaptopResponse response) {
                        logger.info("laptop rated: id = "
                                + response.getLaptopId() + ", count = "
                                + response.getRatedCount() + ", average = "
        ...
        try {
            for (int i = 0; i < n; i++) {
                RateLaptopRequest request = RateLaptopRequest.newBuilder()
                        ...
                        .build();
                requestObserver.onNext(request);
                logger.info("sent rate-laptop request: id = "
                        + request.getLaptopId() + ", score = ");
            }
        } catch (Exception e) {
            logger.log(Level.SEVERE, "unexpected error: " + e.getMessage());
            requestObserver.onError(e);
